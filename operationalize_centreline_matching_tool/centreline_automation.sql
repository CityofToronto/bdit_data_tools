...
geom geometry := (
	...
	-- USED TO BE levenshtein(LOWER(highway2), LOWER(s.lf_name), 1, 1, 2) < 3
	WHERE levenshtein(LOWER(highway2), LOWER(s.lf_name), 1, 1, 2) < 3
	--levenshtein(LOWER(highway2), LOWER(s.lf_name), 1, 1, 1) < 4
	AND 
	(
	(
...
DROP FUNCTION crosic.get_closest_line(pnt GEOMETRY, multi_line GEOMETRY);

CREATE OR REPLACE FUNCTION crosic.get_closest_line(pnt GEOMETRY, multi_line GEOMETRY) 
...
RETURN QUERY SELECT line_dump.geom AS geom_part, ST_Distance(pnt, line_dump.geom)::FLOAT AS dist, 
...
closest_path_not_merged1 INT := (SELECT path FROM crosic.get_closest_line(oid1_geom,centreline_geom_not_merged));

closest_line_geom1 GEOMETRY := (SELECT geom_part FROM crosic.get_closest_line(oid1_geom, centreline_geom) );
				
closest_line_path1 INT := (SELECT path FROM crosic.get_closest_line(oid1_geom,centreline_geom));
				
closest_line_geom2 GEOMETRY := (SELECT geom_part FROM crosic.get_closest_line(oid2_geom,centreline_geom));
				
closest_line_path2 INT := (SELECT path FROM crosic.get_closest_line(oid2_geom,centreline_geom));

closest_path_not_merged2 INT := (SELECT path FROM crosic.get_closest_line(oid2_geom,centreline_geom_not_merged));
...
new_line1 GEOMETRY := (CASE WHEN direction_btwn1 IS NOT NULL 
			THEN crosic.centreline_case2(direction_btwn1, NULL, metres_btwn1, NULL, closest_line_geom1, ST_MakeLine(oid1_geom, oid2_geom), --ST_MakeLine(oid1_geom, endpoint_line1), 
...
new_line2 GEOMETRY := (
			...
			THEN 
			crosic.centreline_case2(NULL, direction_btwn2, NULL, metres_btwn2, closest_line_geom2, ST_MakeLine(oid2_geom, ST_SetSRID(translate_intersection_point(oid2_geom, metres_btwn2, direction_btwn2), 26917)), --ST_MakeLine(oid1_geom, endpoint_line1), 
			oid2_geom,  oid2_geom)
			...
			-- so this line will be from oid2 to the end/start of newline1
			WHEN direction_btwn2 IS NULL AND closest_line_path2 = closest_line_path1 
...
final_line GEOMETRY :=  (
		...
		(
			...
			-- does not work because of line merge ... small lines being merged together and then cut
			
			
			--SELECT path[1], geom 
			---FROM St_Dump(centreline_geom_not_merged) AS dump
			---WHERE path[1] <> closest_path_not_merged1 AND path[1] <> closest_path_not_merged2
			--AND ST_INTERSECTS(dump.geom, new_line1) AND ST_INTERSECTS(dump.geom, new_line2)
			--AND NOT -- buffer around newlines do not intersect with dump.geom ?????
...
RAISE NOTICE 'CUT MULTILINE FUNCTION centreline_geom: %  line_geom % closest_line_path1: % closest_line_path2: % endpoint_line1 % endpoint_line2 % new_line1 % new_line2 % final_line %',
ST_ASText(ST_Transform(centreline_geom, 4326)), ST_AsText(ST_Transform(line_geom, 4326)), closest_line_path1, closest_line_path2,
ST_AsText(ST_Transform(endpoint_line1, 4326)), ST_AsText(ST_Transform(endpoint_line2, 4326)), ST_AsText(ST_Transform(new_line1, 4326)), 
...
geom geometry := (
	...
	WHEN direction_btwn2 IS NULL
		...
		-- intersection occurs before the rough line
		WHEN ST_LineLocatePoint(centreline_geom, oid2_geom)
		< ST_LineLocatePoint(centreline_geom, ST_ClosestPoint(centreline_geom, ST_LineSubstring(line_geom, 0, 0.000001)))  --- ST_LineSubstring(line_geom, 0.99999, 1)))  --
			THEN 
			line_substring_lower_value_first(centreline_geom, 
...
RETURNS TABLE(centreline_segments geometry, con TEXT, notice TEXT, line_geom geometry, oid1_geom geometry, oid2_geom geometry) AS $$ 
DECLARE 
	...
	gis.abbr_street(regexp_REPLACE(regexp_REPLACE(regexp_REPLACE(split_part(split_part(regexp_REPLACE(regexp_REPLACE(frm, '[0123456789.,]* metres (north|south|east|west|East|northeast|northwest|southwest|southeast) of ', '', 'g'), '\(.*?\)', '', 'g'), ' to ', 1), ' and ', 1), '\(.*\)', '', 'g'), '[Bb]etween ', '', 'g'), 'A point', '', 'g'))
	ELSE gis.abbr_street(regexp_REPLACE(regexp_REPLACE(regexp_REPLACE(frm, '\(.*?\)', '', 'g'), '[0123456789.,]* metres (north|south|east|west|East|northeast|northwest|southwest|southeast) of ', '', 'g'), 'A point', '', 'g')) 
	END; 
	...
	btwn2_orig_v1 TEXT := CASE WHEN t IS NULL THEN 
			...
			THEN gis.abbr_street(regexp_REPLACE(regexp_REPLACE(split_part(regexp_REPLACE(regexp_REPLACE(frm, '\(.*?\)', '', 'g'), '[0123456789.,]* metres (north|south|east|west|East|east/north|northeast|northwest|southwest|southeast|south west) of ', '', 'g'), ' and ', 2), '[Bb]etween ', '', 'g'), 'A point', '', 'g'))
			WHEN split_part(frm, ' to ', 2) <> ''
			THEN gis.abbr_street(regexp_REPLACE(regexp_REPLACE(split_part(regexp_REPLACE(regexp_REPLACE(frm, '\(.*?\)', '', 'g'), '[0123456789.,]* metres (north|south|east|west|East|east/north|northeast|northwest|southwest|southeast|south west) of ', '', 'g'), ' to ', 2), '[Bb]etween ', '', 'g'), 'A point', '', 'g'))
			END)
			
			ELSE 
			gis.abbr_street(regexp_REPLACE(regexp_REPLACE(t, '\(.*?\)', '', 'g'), '[0123456789.]* metres (north|south|east|west|East|northeast|northwest|southwest|southeast|south west) of ', '', 'g'))
			END ; 
	...
	direction_btwn1 TEXT := CASE WHEN t IS NULL THEN 
				...
				OR gis.abbr_street( regexp_REPLACE(split_part(split_part(frm, ' to ', 1), ' and ', 1), '[Bb]etween ', '', 'g')) LIKE '% m %'
				THEN split_part(split_part(gis.abbr_street(regexp_REPLACE(regexp_REPLACE(regexp_REPLACE(split_part(split_part(frm, ' to ', 1), ' and ', 1), '[Bb]etween ', '', 'g'), 'further ', '', 'g'), 'east/north', 'northeast', 'g')), ' m ', 2), ' of ', 1)
				ELSE NULL
				END )
				ELSE 
				(
				...
				THEN regexp_replace(regexp_replace(split_part(split_part(gis.abbr_street(frm), ' m ', 2), ' of ', 1), 'further ', '', 'g'), 'east/north', 'northeast', 'g')
				ELSE NULL
				END )
				END;
	...
	direction_btwn2 TEXT := CASE WHEN t IS NULL THEN (
				...
				(
					CASE WHEN split_part(frm, ' and ', 2) <> ''
					THEN regexp_REPLACE(regexp_replace(split_part(split_part( gis.abbr_street(regexp_REPLACE(split_part(frm, ' and ', 2), '[Bb]etween ', '', 'g')), ' m ', 2), ' of ', 1), 'further ', '', 'g'), 'east/north', 'northeast', 'g')
					WHEN split_part(frm, ' to ', 2) <> ''
					THEN regexp_REPLACE(regexp_replace(split_part(split_part(gis.abbr_street(regexp_REPLACE(split_part(frm, ' to ', 2), '[Bb]etween ', '', 'g')), ' m ', 2), ' of ', 1), 'further ', '', 'g'), 'east/north', 'northeast', 'g')
					END
				)
				ELSE NULL
				END)
	...
	metres_btwn1 FLOAT :=	(CASE WHEN t IS NULL THEN 
				...
				THEN regexp_REPLACE(regexp_REPLACE(regexp_REPLACE(split_part(gis.abbr_street(regexp_REPLACE(split_part(split_part(frm, ' to ', 1), ' and ', 1), '[Bb]etween ', '', 'g')), ' m ' ,1), 'a point ', '', 'g'), 'A point', '', 'g'), ',', '', 'g')::FLOAT
				ELSE NULL
				END
				)
				ELSE 
				...
				CASE WHEN btwn1 LIKE '% m %'
				OR gis.abbr_street(frm) LIKE '% m %'
				THEN regexp_REPLACE(regexp_REPLACE(regexp_REPLACE(split_part(gis.abbr_street(frm), ' m ' ,1), 'a point ', '', 'g'), 'A point', '', 'g'), ',', 'g')::FLOAT
				ELSE NULL
				END
				)
				END)::FLOAT;
	...
	centreline_segments geometry := (
				...
				(
				SELECT * 
				FROM ST_LineMerge(crosic.match_line_to_centreline(line_geom, highway2, metres_btwn1, metres_btwn2))
				)
				-- special case 1
				WHEN btwn1 = btwn2
				THEN 
				(
				crosic.centreline_case1(direction_btwn2, metres_btwn2, ST_MakeLine(ST_LineMerge(crosic.match_line_to_centreline(line_geom, highway2, metres_btwn1, metres_btwn2))), line_geom, 
				...
				ELSE 
				(
				crosic.centreline_case2(direction_btwn1, direction_btwn2, metres_btwn1, metres_btwn2, crosic.match_line_to_centreline(line_geom, highway2, metres_btwn1, metres_btwn2), line_geom, 
...
RETURN QUERY (SELECT centreline_segments, con, notice, line_geom, oid1_geom, oid2_geom);
